...
function Rongo:new(AppId: string,Key: string)
    ...
    function Client:SetVersion(Version: "v1" | "beta")
        URI = "https://data.mongodb-api.com/app/%s/endpoint/data/"..Version
    end

    function Client:GetCluster(Name: string)
        local Cluster = {}
        ...
        function Cluster:GetDatabase(Name: string)
            local Database = {}
            ...
            function Database:GetCollection(Name: string)
                local Collection = {}
                ...
                function Collection:FindMany(Filter: {[string]: string | {[string]: string}}?,Limit: number?,Sort: {any?}?,Skip: number?): {[number]: {[string]: any?}}?
                    local RequestData = {
                        ["dataSource"] = self["Cluster"]["Name"],
                        ["database"] = self["Database"]["Name"],
                        ["collection"] = self["Name"],
                ...
                function Collection:InsertMany(Documents: {[number]: {[string]: any?}}): {[number]: string}?
                ...
                function Collection:DeleteOne(Filter: string): number?
                    if not Filter then warn("[RONGO] Filter argument cannot be empty") return nil end
                    local RequestData = {
                        ["dataSource"] = self["Cluster"]["Name"],
                        ["database"] = self["Database"]["Name"],
                ...
                function Collection:DeleteMany(Filter: string): number?
                    if not Filter then warn("[RONGO] Filter argument cannot be empty") return nil end
                    local RequestData = {
                        ["dataSource"] = self["Cluster"]["Name"],
                        ["database"] = self["Database"]["Name"],
