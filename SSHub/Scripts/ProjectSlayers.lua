            ...
            if Settings.KillAura.Enabled then
                if not Settings.PlrDied and Plr.Character:FindFirstChild("HumanoidRootPart") and Plr.Character:FindFirstChild("Humanoid") then
                    ...
                    elseif Settings.KillAura.TypeSelected == "Sword" then
                        Hitting=true
                        wait(.1)
                        game:GetService("ReplicatedStorage").Remotes.To_Server.Handle_Initiate_S_:InvokeServer(unpack(args(1,"Sword_Combat_Slash", 7)))
                        for i = 1, 7 do
                    ...
                    elseif Settings.KillAura.TypeSelected == "Scythe" then
                        Hitting=true
                        wait(.1)
                        game:GetService("ReplicatedStorage").Remotes.To_Server.Handle_Initiate_S_:InvokeServer(unpack(args(1,"Scythe_Combat_Slash", 7)))
                        for i = 1, 7 do
                    ...
                    elseif Settings.KillAura.TypeSelected == "Claws" then
                        Hitting=true
                        wait(.1)
                        game:GetService("ReplicatedStorage").Remotes.To_Server.Handle_Initiate_S_:InvokeServer(unpack(args(1,"claw_Combat_Slash", 7)))
                        for i = 1, 7 do
...
local function lookAt(Target)
	if Plr.Character.PrimaryPart and Target then
		Plr.Character:SetPrimaryPartCFrame(CFrame.new(Plr.Character.PrimaryPart.Position,Target.Position))
	end
end
if World == 1 or World == 2 then
    coroutine.wrap(function()
        ...
        while task.wait() do
            pcall(function()
                if Settings.MugenTrain then
                    ...
                    if Settings.MugenTrain and Enemy:FindFirstChild("HumanoidRootPart") and Enemy:FindFirstChild("Humanoid") and Enemy.Humanoid.Health > 0 and not Settings.PlrDied then
                        if Tween then Tween:Cancel() Tween = nil end
                            ...
                            pcall(function()
                                if Enemy.Humanoid and Enemy.Humanoid:IsDescendantOf(workspace) and Enemy.Humanoid.Health > 0 and not Settings.PlrDied then
                                    ...
                                    if not Hitting and Settings.MugenTrain then
                                        Plr.Character.HumanoidRootPart.CFrame = Enemy.HumanoidRootPart.CFrame*CFrame.new(0,0,-6)
                                    elseif Hitting and Settings.MugenTrain then
                                        Plr.Character.HumanoidRootPart.CFrame = Enemy.HumanoidRootPart.CFrame*CFrame.new(0,0,-6) lookAt(Enemy.HumanoidRootPart)
                                    end
                        ...
                        if not Settings.MugenTrain or Settings.PlrDied then
                            Settings.KillAura.Enabled = false
                            FarmingPos = nil
                            Noclip = false
                            NoFall:Destroy()
...
elseif World == 1 or World == 2 then
coroutine.wrap(function()
    local NoFall
    while task.wait() do
        pcall(function()
                        ...
                        pcall(function()
                            if Enemy.Humanoid and Enemy.Humanoid:IsDescendantOf(workspace) and Enemy.Humanoid.Health > 0 and not Settings.PlrDied then
                                ...
                                elseif Hitting and Settings.NearestEnemyFarm then
                                    Plr.Character.HumanoidRootPart.CFrame = Enemy.HumanoidRootPart.CFrame*CFrame.new(0, 7, 0) lookAt(Enemy.HumanoidRootPart)
                                end
                            elseif not Enemy.Humanoid:IsDescendantOf(workspace) or Enemy.Humanoid.Health <= 0 or Settings.PlrDied then
                                Settings.KillAura.Enabled = false
                                FarmingPos = nil
                                                    ...
                                                    pcall(function()
                                                        if v.Humanoid and v.Humanoid:IsDescendantOf(workspace) and v.Humanoid.Health > 0 and not Settings.PlrDied then
                                                            ...
                                                            elseif Hitting and Settings.SelectedBossFarm then
                                                                Plr.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame*CFrame.new(0, 7, 0) lookAt(v.HumanoidRootPart)
                                                            end
...
coroutine.wrap(function()
    ...
    while task.wait() do
        pcall(function()
                            ...
                            for i2,v2 in pairs(Bosses) do
                                if Settings.FarmAllBosses and not Settings.SelectedBossFarm then
                                    if string.match(v.Name, i2) then
                                        ...
                                        if v:FindFirstChild("HumanoidRootPart") and v.Humanoid and v.Humanoid.Health > 0 and not Settings.PlrDied and Settings.FarmAllBosses and not Settings.SelectedBossFarm and Settings.AllBossesType == "List" then
                                            if Tween then Tween:Cancel() Tween = nil end
                                            TeleportTween(v.HumanoidRootPart.CFrame)
                                            if not Plr.Character.HumanoidRootPart:FindFirstChildOfClass("BodyVelocity") then
                                                NoFall = Instance.new("BodyVelocity", Plr.Character.HumanoidRootPart)
            ...
            elseif Settings.FarmAllBosses and not Settings.SelectedBossFarm and Settings.AllBossesType == "Closest" then
                local Enemy, Pos = ClosestBoss()
                if not Enemy:FindFirstChild("HumanoidRootPart") and Enemy:FindFirstChild("Humanoid") and Enemy.Humanoid.Health > 0 and not Settings.PlrDied and Settings.FarmAllBosses and not Settings.SelectedBossFarm then
                    if Tween then Tween:Cancel() Tween = nil end
                    TeleportTween(Pos)
                end
                if Enemy:FindFirstChild("HumanoidRootPart") and Enemy.Humanoid and Enemy.Humanoid.Health > 0 and not Settings.PlrDied and Settings.FarmAllBosses and not Settings.SelectedBossFarm and Settings.AllBossesType == "Closest" then
                    if Tween then Tween:Cancel() Tween = nil end
                    TeleportTween(Enemy.HumanoidRootPart.CFrame)
                    if not Plr.Character.HumanoidRootPart:FindFirstChildOfClass("BodyVelocity") then
                        NoFall = Instance.new("BodyVelocity", Plr.Character.HumanoidRootPart)
                        ...
                        while task.wait() and Settings.CivilianFarm and Enemy.Humanoid and Enemy.Humanoid:IsDescendantOf(workspace) and Enemy.Humanoid.Health > 0 and not Settings.PlrDied do
                            if Enemy.Humanoid and Enemy.Humanoid:IsDescendantOf(workspace) and Enemy.Humanoid.Health > 0 and not Settings.PlrDied then
                                ...
                                elseif Hitting and Settings.CivilianFarm then
                                    Plr.Character.HumanoidRootPart.CFrame = Enemy.HumanoidRootPart.CFrame*CFrame.new(0, 0, -2) lookAt(Enemy.HumanoidRootPart)
                                end
                            elseif not Enemy.Humanoid or not Enemy.Humanoid:IsDescendantOf(workspace) or Enemy.Humanoid.Health <= 0 or Settings.PlrDied then
                                Settings.KillAura.Enabled = false
                                FarmingPos = nil
...
local InfoTab = SShub:AddTab("⚠️ Info ⚠️")
local Visuals = SShub:AddTab("Visuals")
local MainInfo = InfoTab:AddLeftGroupbox("Important Info Please Read")
if World == 1 or World == 2 then
    local Teleports = SShub:AddTab("Teleports")
    local MainTrz = Teleports:AddLeftTabbox("Teleports")
    MainT = MainTrz:AddTab("Locations")
    MainT1 = MainTrz:AddTab("Breaths")
    MainTele = Teleports:AddRightGroupbox("Teleports")
    MainTr = Teleports:AddLeftGroupbox("Options")
end
local Settingss = SShub:AddTab("Settings")
if World == 1 or World == 2 then
    local MainCoo = Farm:AddLeftTabbox("Farming")
    MainCo = MainCoo:AddTab("Bosses")
    MainC1 = MainCoo:AddTab("Demon & Npc")
    MainC2 = MainCoo:AddTab("Options")
    MainC3 = Farm:AddRightGroupbox("Spawned Bosses")
elseif World == 3 then
    local MainCoo1 = Farm:AddLeftTabbox("Mugen Train")
    MainCo1 = MainCoo1:AddTab("Mugen Farm")
    MainC21 = MainCoo1:AddTab("Options")
end
...
elseif World == 3 then
    MainCo1:AddToggle('MugenTrain', {Text = 'Mugen Train',Default = Settings.MugenTrain,Tooltip = 'MugenTrain'})
...
MainInfo:AddDivider()
MainInfo:AddLabel("PLEASE DON'T ACTIVE 2 FARMS OR MORE AT THE SAME TIME OR THE SCRIPT MAY BREAK",true)
MainInfo:AddDivider()
MainInfo:AddLabel("DONT UNFOCUS ROBLOX WHILE AUTOFARMING OR ABOUT 1m - 10m U GONA GET KICKED BY THE ANTICHEAT (Probaly)",true)
MainInfo:AddDivider()
MainInfo:AddLabel("If autofarms dont working correctly u can go to\nOptions > Bring Mob it may will fix it a bit.\nBe sure no one is close to boss.\nU can try change farm altitude in Options too.",true)
MainInfo:AddDivider()
local BreathCheck = MainCr:AddLabel("...")
local DemonCheck = MainCr:AddLabel("...")
if World == 1 or World == 2 then
    MainC3:AddToggle('AliveBossesList', {Text = 'Update Alive Bosses List',Default = Settings.AliveBossesList,Tooltip = ''})
...
if World == 1 or World == 2 then
    if World == 1 then
        BossCheckLabel = MainC3:AddLabel("Loading...")
        BossCheckLabel1 = MainC3:AddLabel("Loading...")
        BossCheckLabel2 = MainC3:AddLabel("Loading...")
        BossCheckLabel3 = MainC3:AddLabel("Loading...")
        BossCheckLabel4 = MainC3:AddLabel("Loading...")
        BossCheckLabel5 = MainC3:AddLabel("Loading...")
        BossCheckLabel6 = MainC3:AddLabel("Loading...")
        BossCheckLabel7 = MainC3:AddLabel("Loading...")
        BossCheckLabel8 = MainC3:AddLabel("Loading...")
        BossCheckLabel9 = MainC3:AddLabel("Loading...")
    elseif World == 2 then
        BossCheckLabel = MainC3:AddLabel("Loading...")
        BossCheckLabel1 = MainC3:AddLabel("Loading...")
        BossCheckLabel2 = MainC3:AddLabel("Loading...")
...
if World == 1 or World == 2 then
    MainC2:AddDropdown('FarmMethod', {Values = {"Fists","Sword","Scythe","Claws"}, Default = 1, Multi = false, Text = 'Auto Farms Method (Mastery)', Tooltip = 'Select farm method, it will up mastery on the selected one'})
        Options.FarmMethod:OnChanged(function()
            Settings.KillAura.TypeSelected = Options.FarmMethod.Value
    end)
    MainC2:AddDropdown('AllBossesType', {Values = {"List","Closest"}, Default = 1, Multi = false, Text = 'Farm All Bosses Method', Tooltip = 'Select ur Farm Method'})
        Options.AllBossesType:OnChanged(function()
            Settings.AllBossesType = Options.AllBossesType.Value
    end)
    MainC2:AddSlider('FarmAltitude', {Text = 'Farm Altitude (Recommended Default)',Default = Settings.FarmAltitude,Min = -150,Max = 150,Rounding = 0,Compact = false})
    Options.FarmAltitude:OnChanged(function()
        Settings.FarmAltitude = Options.FarmAltitude.Value
    end)
    MainC2:AddSlider('ZDirection', {Text = 'Farm Axis or Far (Recommended Default)',Default = Settings.ZDirection,Min = -150,Max = 150,Rounding = 0,Compact = false})
    Options.ZDirection:OnChanged(function()
        Settings.ZDirection = Options.ZDirection.Value
    end)
    MainC2:AddToggle('BringMob', {Text = 'Bring Mob',Default = Settings.BringMob,Tooltip = 'Always bring the mob when farming'})
        Toggles.BringMob:OnChanged(function()
            Settings.BringMob = Toggles.BringMob.Value
    end)
elseif World == 3 then
    MainC21:AddDropdown('FarmMethod', {Values = {"Fists","Sword","Scythe","Claws"}, Default = 1, Multi = false, Text = 'Auto Farms Method (Mastery)', Tooltip = 'Select farm method, it will up mastery on the selected one'})
        Options.FarmMethod:OnChanged(function()
            Settings.KillAura.TypeSelected = Options.FarmMethod.Value
    end)
end
